...
var boostPFSFilterConfig = {
  general: {
    ...
    selectOptionContainer: '.boostPFSFilterConfig',  // CSS selector to append the product option, if left empty it will append to the product item
    separateRefineByFromFilter: true,
    filterTreeIcon: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2 3.5a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1H2Zm0 2.667a.5.5 0 1 0 0 1h2a.5.5 0 0 0 0-1H2Zm-.5 3.166a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1H2a.5.5 0 0 1-.5-.5ZM2 11.5a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1H2ZM5.5 4a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 0 1H6a.5.5 0 0 1-.5-.5ZM6 6.167a.5.5 0 1 0 0 1h4.667a.5.5 0 0 0 0-1H6Zm-.5 3.166a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 0 1H6a.5.5 0 0 1-.5-.5ZM6 11.5a.5.5 0 0 0 0 1h4a.5.5 0 0 0 0-1H6Z" clip-rule="evenodd"/></svg>',
    filterTreeIconClose: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 16 16"><path fill="currentColor" fill-rule="evenodd" d="M2 3.5a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1H2Zm0 2.667a.5.5 0 1 0 0 1h2a.5.5 0 0 0 0-1H2Zm-.5 3.166a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 0 1H2a.5.5 0 0 1-.5-.5ZM2 11.5a.5.5 0 0 0 0 1h2a.5.5 0 0 0 0-1H2ZM5.5 4a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 0 1H6a.5.5 0 0 1-.5-.5ZM6 6.167a.5.5 0 1 0 0 1h4.667a.5.5 0 0 0 0-1H6Zm-.5 3.166a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 0 1H6a.5.5 0 0 1-.5-.5ZM6 11.5a.5.5 0 0 0 0 1h4a.5.5 0 0 0 0-1H6Z" clip-rule="evenodd"/></svg>',
    breakpointMobile: '299'
  },
  selector: {
    otpButtons: '.product-card__figure',
  }
};
...
/************************** BUILD PRODUCT LIST **************************/
  ...
  ProductGridItem.prototype.compileTemplate = function(data) {
    ...
    // Add Regions
    itemHtml = itemHtml.replace(/{{itemRegions}}/g, buildRegions(data));
    ...
    itemHtml = itemHtml.replace(/{{itemTitle}}/g, buildTitle(data));
...
/************************** BUILD PRODUCT ITEM ELEMENTS **************************/
  function buildGridWidthClass(data) {
    var gridWidthClass = 'product-card product-card--blends product-card--show-secondary-media';
    data.tags.forEach(tag => {
      switch (tag) {
        case 'High Poly':
          gridWidthClass += ' product-card--high-poly';
          break;
        case 'Low Poly':
          gridWidthClass += ' product-card--low-poly';
          break;
        case '2D Images':
          gridWidthClass += ' product-card--2d-images';
          break;
      }
    });
  ...
  function buildImages(data) {
    ...
    html += '<a href="{{itemUrl}}">';
    html += '<img class="product-card__image product-card__image--primary lazyload Image--lazyLoad"' +
    ...
    if (activeSwapImage) {
      html += '<img class="product-card__image product-card__image--secondary object-fill lazyload Image--lazyLoad"' +
  ...
  function buildRegions(data) {
    let html = '<ul class="product-tags">';
    data.tags.forEach((tag) => {
  ...
  function buildPrice(data) {
    ...
    if (boostPFSThemeConfig.custom.hasOwnProperty('show_price') &&
      ...
      html = '<price-list class="price-list">';
      if (onSale) {

        html += '<sale-price class="text-on-sale"><span class="sr-only">Sale price</span>' + Utils.formatMoney(data.price_min) + '</sale-price>';
        html += '<compare-at-price class="text-subdued line-through"><span class="sr-only">Regular price</span>' + Utils.formatMoney(data.compare_at_price_min) + '</compare-at-price> ';
      } else {
        if (priceVaries) {
  ...
  function buildTagLabels(data) {
    let tagLabel = '';

    for (let i = 0; i < data.tags.length; i++) {
      if(data.tags[i].includes('Low Poly')) {
        tagLabel = `<div class="product-tag tag-low">
                      <span>Low Poly</span>
                    </div>`;
        break;
      } else if(data.tags[i].includes('2D Images')) {
        tagLabel = `<div class="product-tag tag-2d">
                      <span>2D Images</span>
                    </div>`;
        break;
...
/************************** END BUILD TOOLBAR **************************/
  ...
  Filter.prototype.afterRender = function(data) {
    if (!data) data = this.data;
    var totalProduct = data.total_product + ' ' + boostPFSThemeConfig.label.items_with_count_other;
    if (data.total_product == 1) {
      totalProduct = data.total_product + ' ' + boostPFSThemeConfig.label.items_with_count_one;
    }
  ...
  FilterTree.prototype.afterRender = function() {
    // Show number results for.
    let resultItems = parseInt(jQ(this.$element.find(this.selector.numberResult)[0]).text().replace(/\(|\)/g, ''));

    let totalProductText = '<strong>' + resultItems + '</strong> ' + boostPFSThemeConfig.label.items_with_count_other + ' for';
    if (resultItems == 1) {
      totalProductText = '<strong>' + resultItems + '</strong> ' + boostPFSThemeConfig.label.items_with_count_one + ' for';
    }

    jQ('.boost-pfs-filter-refine-by > span').html(totalProductText);
  };

  let originalBuildLabelFunction = FilterOptionItem.prototype.buildLabel;

  FilterOptionItem.prototype.buildLabel = function() {
    let label = originalBuildLabelFunction.call(this);

    if (label == 'Tree') label = 'Trees';
    if (label == 'Shrub') label = 'Shrubs';
